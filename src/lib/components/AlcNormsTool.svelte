<script>
	...
	$effect(() => {
		// Reset slider when measure changes (tracks `measure` as dependency)
		void measure;
	...
	const chartBars = $derived.by(() => {
		...
		for (let i = 0; i < cdf.length - 1; i++) {
			...
			const v1 = cdf[i + 1].v;
			if (v1 > maxD) break;
			const prop = cdf[i + 1].p - cdf[i].p;
			const width = v1 - v0;
			if (width <= 0 || prop < 0.0001) continue;
			const density = prop / width;
			if (density > maxDensity) maxDensity = density;
			raw.push({ x0: v0, x1: v1, prop, density });
		}
		...
		return raw.map((b) => {
			...
			return {
				...
				svgW: Math.max(((b.x1 - b.x0) / maxD) * SVG_W - 2, 1),
				svgH: maxDensity > 0 ? (b.density / maxDensity) * SVG_H : 0,
				highlighted: userValue > 0 && mid <= userValue
			};
		});
	});
...
<button class="launch-btn" onclick={open}>How do I compare? &rarr;</button>
...
{#if isOpen}
	...
	>
		<div class="modal-header">
			<h2 id="alc-title">Alcohol Use: Population Norms</h2>
		...
		<div class="modal-body">
			...
			{:else}
				...
				<div class="measure-tabs">
					{#each Object.entries(MEASURES) as [key, cfg]}
						<button
							class="measure-tab"
							class:active={measure === key}
							onclick={() => (measure = key)}>{cfg.shortLabel}</button
						>
					{/each}
				</div>
				...
				{#if groupData}
					<!-- Stats block: orange top bar + green sub-bar, above chart -->
					<div class="stats-block">
						{#if percentile !== null && userValue > 0}
							<div class="feedback">
								<div class="feedback-stat">
									<span class="pct-big">{formatPct(percentile)}</span>
									<span class="pct-label">of {getDemoLabel()} {getFeedbackText(userValue)}</span>
								</div>
								{#if percentile >= 0.95}
									<div class="high-note">
						...
						>
							{#each chartBars as bar}
								<rect
									...
									width={bar.svgW}
									height={bar.svgH}
									fill={bar.highlighted ? '#f59e0b' : '#e0e0e0'}
									rx="2"
								/>
							{/each}
					...
					<!-- Slider -->
					<div class="input-row">
						<label class="input-label" for="alc-slider">Value</label>
				...
				<p class="footnote">
					Based on 2021–2024 NSDUH data (SAMHSA), n&nbsp;&#8776;&nbsp;232,000. Self-reported survey
					data systematically underestimates actual consumption — research suggests the heaviest 10%
					of drinkers consume far more than surveys capture. These percentiles reflect what people
					<em>report</em>, not verified intake. All calculations happen in your browser; no data is
					collected or stored.
				</p>
			{/if}
		</div>
